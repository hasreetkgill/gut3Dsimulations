...
%           Explicit model for hindgut transversely isotropic
...
data_name = 'SY_TR02RR065MR21_Circu1_Tet_Explicit'; Radius = 5.4; 
...
node_num = length(M)        % Total numer of the nodes
...
[Z_value, start, floor_row] = unique(Z_node_sort,'rows'); % Separate theta-level
 Z_floor = length(Z_value)
floor_num = node_num/Z_floor % Numer of nodes in each floor
mesh_row = Z_floor;
...
Length_th = min(max(Theta_node_sort),abs(min(Theta_node_sort)))
...
fig1 = figure(1)
set(fig1,'color',[1 1 1]); % White backgraoud
...
Ur_grid_double = [flip(Ur_grid(2:end,:));Ur_grid];                     size(Ur_grid_double);
...
%%    FFT analysis for displacement field--02/01/2023 by Sifan 
N_th = 256; L_th = Length_th*2;
N_z = 256; L_z = Length_Z*2;
